...
def data_loader(filename, property_value_keys):
    ...
    # drop those rows that have no value in all the property columns
...
def search_the_db(search_id, min_value, max_value):
    ...
    if property_no == 1:
        result_obj = MaterialProperties.objects.filter(
            melting_point__gte=min_value, melting_point__lte=max_value).order_by('-melting_point')
    elif property_no == 2:
        result_obj = MaterialProperties.objects.filter(
            thermal_conductivity__gte=min_value, thermal_conductivity__lte=max_value).order_by('-thermal_conductivity')
    elif property_no == 3:
        result_obj = MaterialProperties.objects.filter(
            density__gte=min_value, density__lte=max_value).order_by('-density')
    elif property_no == 4:
        result_obj = MaterialProperties.objects.filter(
            specific_heat__gte=min_value, specific_heat__lte=max_value).order_by('-specific_heat')
    else:
        result_obj = ''
